...
class DistortionRenderer
{
    ...
    func renderDistortionMesh(_ mesh:DistortionMesh, textureID:GLint)
    {
        ...
        glVertexAttribPointer(GLuint(holder.positionLocation), 2, GLenum(GL_FLOAT), GLboolean(GL_FALSE), GLsizei(9 * MemoryLayout<Float>.size), BUFFER_OFFSET(0))
        glEnableVertexAttribArray(GLuint(holder.positionLocation))
        
        glVertexAttribPointer(GLuint(holder.vignetteLocation), 1, GLenum(GL_FLOAT), GLboolean(GL_FALSE), GLsizei(9 * MemoryLayout<Float>.size), BUFFER_OFFSET(2*MemoryLayout<Float>.size))
        glEnableVertexAttribArray(GLuint(holder.vignetteLocation))
        
        glVertexAttribPointer(GLuint(holder.blueTextureCoordLocation),
                              2,
                              GLenum(GL_FLOAT),
                              GLboolean(GL_FALSE),
                              GLsizei(9 * MemoryLayout<Float>.size),
                              BUFFER_OFFSET(7*MemoryLayout<Float>.size))
        
        glEnableVertexAttribArray(GLuint(holder.blueTextureCoordLocation))
        
        if chromaticAberrationCorrectionEnabled
        {
            glVertexAttribPointer(GLuint(holder.redTextureCoordLocation), 2, GLenum(GL_FLOAT), GLboolean(GL_FALSE), GLsizei(9 * MemoryLayout<Float>.size), BUFFER_OFFSET(3*MemoryLayout<Float>.size))
            glEnableVertexAttribArray(GLuint(holder.redTextureCoordLocation))
            glVertexAttribPointer(GLuint(holder.greenTextureCoordLocation), 2, GLenum(GL_FLOAT), GLboolean(GL_FALSE), GLsizei(9 * MemoryLayout<Float>.size), BUFFER_OFFSET(5*MemoryLayout<Float>.size))
             ...
             _ vignetteEnabled:Bool)
        {
            ...
            for row in 0..<rows-1
            {
                if row > 0
                {
                    ...
                    indexOffset = indexOffset.advanced(by: 1)
                }
                for col in 0..<cols
                {
                    if col > 0
